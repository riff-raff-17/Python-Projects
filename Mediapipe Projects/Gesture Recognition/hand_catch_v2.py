'''Implemented difficulty scaling and random ball speeds'''
...
def main():
    circles = []
    speeds = []  # List to store individual speeds for each ball
    score = 0
    lives = MAX_LIVES
    game_active = False
    frame_counter = 0
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            cx, cy = int(hand_landmarks.landmark[mp.solutions.hands.HandLandmark.INDEX_FINGER_TIP].x * WIDTH), int(hand_landmarks.landmark[mp.solutions.hands.HandLandmark.INDEX_FINGER_TIP].y * HEIGHT)
        ...
        if frame_counter >= BALL_GENERATION_INTERVAL:
            ...
            speeds.append(random.uniform(3, 7))  # Assign random speed to each ball
            frame_counter = 0
        ...
        for i, circle in enumerate(circles):
            ...
            if catcher_rect.colliderect(pygame.Rect(circle[0] - CIRCLE_RADIUS, circle[1] - CIRCLE_RADIUS, CIRCLE_RADIUS * 2, CIRCLE_RADIUS * 2)):
                ...
                catch_sound.play()
                score += 1
                break  # Avoid index errors after modifying the list
