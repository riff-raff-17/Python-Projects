'''
Controlling the robot using the index finger. 
Connects using laptop camera.
'''
...
while True: 
    ...
    nparr = np.frombuffer(frame2, np.uint8)
    data = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
  
    # Flip image 
    frame2 = cv2.flip(data, 1)
    ...
    if Process.multi_hand_landmarks:
        ...
        if landmarkList:
            ...
            min_distance_threshold = 600  # Adjust based on camera setup
    ...
    if landmarkList:
        distance = int(calculate_distance(index_tip, hand_center))
    else:
        distance = "None"
    # Display the detected direction on screen
    cv2.putText(frame, f"{direction} | {distance}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
